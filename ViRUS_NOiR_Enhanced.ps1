...
# Version 3.0 - The King of Cyber Defense - ENHANCED EDITION
...
$VirusTotalAPI = "YOUR_API_KEY_HERE"  # Get from https://www.virustotal.com/
$AbuseIPDBAPI = "YOUR_API_KEY_HERE"   # Get from https://www.abuseipdb.com/
...
function Initialize-Logs {
    ...
    "$timestamp : ViRUS NOiR Enhanced Started" | Out-File -FilePath $LogFile -Append
}

# Enhanced Administrator Check with Auto-Elevation
function Test-Administrator {
    $currentPrincipal = New-Object Security.Principal.WindowsPrincipal([Security.Principal.WindowsIdentity]::GetCurrent())
    if (-not $currentPrincipal.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
        Write-Host "[!] ViRUS NOiR requires Administrator privileges!" -ForegroundColor Red
        Write-Host "[*] Auto-elevating to Administrator..." -ForegroundColor Yellow
        Start-Process PowerShell -ArgumentList "-NoProfile -ExecutionPolicy Bypass -File `"$PSCommandPath`" $Command" -Verb RunAs
        exit
    }
    return $true
}
...
function Show-Banner {
    ...
    Write-Host "      ╚═══╝  ╚═╝╚═╝  ╚═╝ ╚═════╝ ╚══════╝    ╚═╝  ╚═══╝ ╚═════╝ ╚═╝╚═╝  ╚═╝" -ForegroundColor Magenta
    Write-Host "" -ForegroundColor Magenta
    Write-Host "           ULTIMATE CYBER DEFENSE SYSTEM v3.0 - ENHANCED PROTECTION" -ForegroundColor Cyan
...
# Enhanced Firewall with Geo-Blocking
function Invoke-FirewallConfiguration {
    ...
    try {
        ...
        # Block potentially dangerous outbound
        New-NetFirewallRule -DisplayName "ViRUS_NOiR_Block_PowerShell_Out" -Direction Outbound -Program "C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe" -Action Block -ErrorAction SilentlyContinue
        New-NetFirewallRule -DisplayName "ViRUS_NOiR_Block_CMD_Out" -Direction Outbound -Program "C:\Windows\System32\cmd.exe" -Action Block -ErrorAction SilentlyContinue
    } catch {
...
function Invoke-MalwareHunter {
    ...
    # Enhanced process analysis with behavioral detection
...
# Enhanced Real-time Monitoring with AI Detection
function Start-RealtimeMonitoring {
    ...
    $monitorScript = @"
    `$ThreatLog = "$ThreatLog"
    `$suspiciousPatterns = @("185.", "45.", "5.188.", "91.243.", "192.168.", "10.")
    ...
    while (`$true) {
        ...
        Get-NetTCPConnection | Where-Object {`$_.State -eq "Established"} | ForEach-Object {
            foreach (`$pattern in `$suspiciousPatterns) {
                if (`$_.RemoteAddress -like "`$pattern*") {
        ...
        Get-ScheduledTask | Where-Object {`$_.State -eq "Ready"} | ForEach-Object {
            ...
            if ((Get-Date) - `$created -lt (New-TimeSpan -Minutes 5)) {
                "`$timestamp : NEW SCHEDULED TASK: `$(`$task.TaskName)" | Out-File -FilePath `$ThreatLog -Append
            }
        }
...
# Enhanced Emergency Lockdown with Isolation
...
function Invoke-ThreatIntelligence {
    ...
    foreach ($malware in $knownMalicious) {
        if (Get-Process -Name $malware.Replace('.exe','') -ErrorAction SilentlyContinue) {
...
function Show-MainMenu {
    do {
        ...
        Write-Host "12) Exit ViRUS NOiR" -ForegroundColor Gray
        Write-Host ""
        ...
        switch ($choice) {
            ...
            "11" {
                if (Test-Path $ThreatLog) {
                    Write-Host "`nThreat Log (Last 20 entries):" -ForegroundColor Cyan
            ...
            "12" {
                Write-Host "[+] ViRUS NOiR Enhanced - Maximum protection activated!" -ForegroundColor Green
...
switch ($Command) {
    ...
    "help" {
        Show-Banner
        Write-Host "Enhanced ViRUS NOiR v3.0 - Usage: .\ViRUS_NOiR_Enhanced.ps1 [COMMAND]"
        Write-Host ""
        Write-Host "Commands:"
